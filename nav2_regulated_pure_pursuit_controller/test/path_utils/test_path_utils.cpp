...
TEST(PathUtils, test_generate_straight)
{
  ...
  auto path = generate_path(
      start, spacing, {std::make_unique<Straight>(path_length)});
...
TEST(PathUtils, test_generate_all)
{
  ...
  auto path = generate_path(
      start, spacing, {
                          std::make_unique<Straight>(1.0), std::make_unique<LeftTurn>(1.0), std::make_unique<RightTurn>(1.0), std::make_unique<LeftTurnAround>(1.0), std::make_unique<RightTurnAround>(1.0), std::make_unique<LeftCircle>(1.0), std::make_unique<RightCircle>(1.0), std::make_unique<Arc>(1.0, 2 * M_PI), // another circle
                      });
