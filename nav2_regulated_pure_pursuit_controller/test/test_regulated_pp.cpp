...
TEST(RegulatedPurePursuitTest, applyConstraints)
{
    ...
    auto no_approach_path = path_utils::generate_path(
        geometry_msgs::msg::PoseStamped(), 0.1, {std::make_unique<path_utils::Straight>(approach_velocity_scaling_dist + 1.0)});
...
TEST(RegulatedPurePursuitTest, testDynamicParameter)
{
    ...
    EXPECT_EQ(
        node->get_parameter(
                "test.max_allowed_time_to_collision_up_to_carrot")
            .as_double(),
        2.0);
    ...
    EXPECT_EQ(
        node->get_parameter(
                "test.use_cost_regulated_linear_velocity_scaling")
            .as_bool(),
        false);
...
TEST_F(TransformGlobalPlanTest, prune_after_leaving_costmap)
{
    ...
    auto global_plan = path_utils::generate_path(
        start_of_path, spacing, {std::make_unique<path_utils::Straight>(path_length), std::make_unique<path_utils::LeftTurnAround>(1.0), std::make_unique<path_utils::Straight>(path_length)});
